...
class WChatScreenController extends GetxController {
  ...
  RxBool get isConnected => wifiController.isConnected;
  ...
  void onInit() async {
    if (Get.arguments is ChatSessionModel) {
      ...
      final foundSession = await HiveService.to.loadChatSession(
        wifiController.connectedDevice?.deviceAddress ?? Uuid().v4(),
      );
      chatSession =
          foundSession ??
          ChatSessionModel(
            id: wifiController.connectedDevice?.deviceAddress ?? Uuid().v4(),
            name:
                wifiController.connectedDevice?.deviceName ??
                wifiController.connectedDevice?.deviceAddress ??
                'Unknown',
            createdAt: DateTime.now(),
            updatedAt: DateTime.now(),
            messages: messages,
            device: {
              'name': wifiController.connectedDevice?.deviceName,
              'address': wifiController.connectedDevice?.deviceAddress,
            },
          );
      messages.value = chatSession.messages;
    }
    ...
    connectionSubscription = wifiController.isConnected.listen((event) {
    ...
    setupCallbacks();
    _disconnectSubscription = wifiController.lastDisconnectReason.listen((reason) {
  ...
  void onClose() {
    ...
    wifiController.disconnectFromDevice();
    connectionSubscription?.cancel();
    _messagesSubscription?.cancel();
    _disconnectSubscription?.cancel();
    wifiController.onChatClosed();
    super.onClose();
  }

  void setupCallbacks() {
    wifiController.onSocketData((bytes, text, {required String kind}) async {
      if (kDebugMode) {
    ...
    wifiController.onTransferProgress(({
      required direction,
      required current,
      required total,
      required kind,
      ...
      if (direction == 'out') {
        wifiController.outgoingTransfer.value = state;
        if (kind == 'bytes') {
          final bool needsNew =
        ...
        wifiController.incomingTransfer.value = state;
        if (kind == 'bytes') {
  ...
  void sendTextMessage() {
    if (isConnected.value) {
      wifiController.sendMessage(textController.text);
  ...
  Future<void> pickAndSendImage() async {
    ...
    try {
      ...
      await wifiController.sendBytes(bytes);
    } catch (_) {
